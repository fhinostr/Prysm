...
function renderHubGroupMembers(group) {
  ...
  if (members.length === 0) {
    ...
    listEl.innerHTML = members.map(mId => {
      ...
      if(!p) return '';
      return `
        <div onclick="if(!event.target.closest('button')) window.location.href='client-hub.html?client=${p.id}'" style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.7); padding:0.4rem 0.5rem; border-radius:8px; border: 1px solid rgba(32,178,170,0.2); cursor:pointer; transition:all 0.2s;" onmouseover="this.style.background='rgba(32,178,170,0.1)'" onmouseout="this.style.background='rgba(255,255,255,0.7)'" title="Go to ${escapeHtml(p.name)}'s Hub">
          ...
          <button class="btn-icon" onclick="hubRemoveGroupMember('${p.id}'); event.stopPropagation();" style="background:none; border:none; cursor:pointer; color:var(--color-red); padding:0.1rem;" title="Remove from cohort">
